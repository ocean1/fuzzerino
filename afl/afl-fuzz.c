...
u64 gen_total,                        /* Total number of generated seeds  */
    ...
    last_seed_time,                   /* ms time since last uniq seed     */
    slow_since;                       /* ms time since gen is slow        */
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (rlen == 4) {
    ...
    if (rlen == sizeof(struct gfz_data)) {
      ...
      /* Allocate and zero-initialize the maps of dry run executions. */

      __gfz_num_dry_execs = calloc(__gfz_num_locs, sizeof(u32));
      __gfz_ptr_dry_execs = calloc(__gfz_ptr_locs, sizeof(u32));
      __gfz_bnc_dry_execs = calloc(__gfz_branch_locs, sizeof(u32));
...
void ban_hammer() {
  ...
  for ( loc = 0; loc < __gfz_num_locs; ++loc ) {
    fault_to_exec_ratio = (double)__gfz_num_ban_map[loc] / __gfz_num_dry_execs[loc];

    fprintf(log_file, "\n(num location %u):\t%u faults out of %u (%.2f).", loc,
      __gfz_num_ban_map[loc], __gfz_num_dry_execs[loc], fault_to_exec_ratio);
  ...
  for ( loc = 0; loc < __gfz_ptr_locs; ++loc ) {
    fault_to_exec_ratio = (double)__gfz_ptr_ban_map[loc] / __gfz_ptr_dry_execs[loc];

    fprintf(log_file, "\n(ptr location %u):\t%u faults out of %u (%.2f).", loc,
      __gfz_ptr_ban_map[loc], __gfz_ptr_dry_execs[loc], fault_to_exec_ratio);
  ...
  for ( loc = 0; loc < __gfz_branch_locs; ++loc ) {
    fault_to_exec_ratio = (double)__gfz_bnc_ban_map[loc] / __gfz_bnc_dry_execs[loc];

    fprintf(log_file, "\n(bnc location %u):\t%u faults out of %u (%.2f).", loc,
      __gfz_bnc_ban_map[loc], __gfz_bnc_dry_execs[loc], fault_to_exec_ratio);
